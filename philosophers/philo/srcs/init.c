...
/*   Updated: 2022/07/15 15:38:02 by faventur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	init_data(int argc, char **argv, t_philo *philo)
{
	...
	if (!philo->data)
		return (0);
	ft_memset(philo->data, 0, sizeof(t_data));
	...
	philo->data->sophiste = malloc(sizeof(int) * philo->data->nb);
	philo->data->meal = malloc(sizeof(long long) * philo->data->nb);
	if (!philo->data->sophiste || !philo->data->meal)
		return (0);
	ft_memset(philo->data->sophiste, 0, sizeof(int) * philo->data->nb);
	ft_memset(philo->data->meal, 0, sizeof(long long) * philo->data->nb);
	return (1);
}
